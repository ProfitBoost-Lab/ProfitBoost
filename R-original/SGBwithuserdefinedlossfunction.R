...
sgb.loss = function(# specify datasets, DVs and covariates
  data.train,
  data.valid,
  data.test,
  otherdata1,
  ...
  pos.y,
  pos.covariates,
  # churn variable, positions (col) of churn and covariates
  pi,
  pi.valid,
  # individual expected profit lifts used as DV and weights in the profit loss function
  ...
  # learning rate
  stoch,
  # use stochastic GB or regular GB
  ratio,
  # fraction of observations to be sampled in case of stochastic GB
  ...
  likfunc = function(myloss, myyt, mypi, myfm) {
    weights = weightfunc(myloss = myloss,
                         myyt = myyt,
                         mypi = mypi)
    margin = marginfunc(
      myloss = myloss,
  ...
  best.iteration.valid = (1:iter)[maxlift.convergence.valid == max(maxlift.convergence.valid, na.rm =
                                                                     TRUE)][1]
