...
create.3samples = function (mydata, size.train, size.valid, size.test, myseed = 268237)
  {
    RNGkind(sample.kind = "Rounding" )  # for reproducible results w.r.t. older versions of R
    set.seed(myseed)
    randomization.index=sample(seq(1,nrow(mydata)),size=nrow(mydata),replace=FALSE) 
    
    data.train = mydata[randomization.index[1:size.train],]
    data.valid = mydata[randomization.index[(size.train+1):(size.train+size.valid)],]
    ...
    return(list(data.train=data.train,data.valid=data.valid,data.test=data.test,
                train.index=train.index,test.index=test.index))
  }