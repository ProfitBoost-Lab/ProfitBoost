...
targetsizeoptimization = function(y, # post-intervention churn dummy
                                  treated, # treatment dummy
                                  scores,  # predictions
                                  m, # post-intervention cash flows
                                  delta, # intervention cost
                                  conditional = TRUE, # conditional offer or not
                                  increment = .1, # share of customers added at a time to build the campaign profit curve
                                  plot = TRUE) # plot the campaign profit curve
{
  ...
  if (increment == "unit") {
    maxprofit.trimmed = max(campaign.profit.curve[5:length(target.sizes)],na.rm=TRUE) 
    ...
    targetsize.maxprofit.trimmed = 
      min(target.sizes[5:length(target.sizes)][maxprofit.trimmed == campaign.profit.curve[5:length(target.sizes)]],na.rm=TRUE) 
    secondbest.targetsize.maxprofit.trimmed = 
      min(target.sizes[5:length(target.sizes)][secondbest.maxprofit.trimmed == campaign.profit.curve[5:length(target.sizes)]],na.rm=TRUE)}
  
  if (increment != "unit") {
    maxprofit.trimmed = max(campaign.profit.curve[max((length(target.sizes)*increment),5):length(target.sizes)],na.rm=TRUE) 
    secondbest.index = rev(order(campaign.profit.curve[max((length(target.sizes)*increment),5):length(target.sizes)],na.last=FALSE))[2]
    ...
    targetsize.maxprofit.trimmed = 
      min(target.sizes[max((length(target.sizes)*increment),5):length(target.sizes)][maxprofit.trimmed == campaign.profit.curve[max((length(target.sizes)*increment),5):length(target.sizes)]],na.rm=TRUE) 
    secondbest.targetsize.maxprofit.trimmed = 
      min(target.sizes[max((length(target.sizes)*increment),5):length(target.sizes)][secondbest.maxprofit.trimmed == campaign.profit.curve[max((length(target.sizes)*increment),5):length(target.sizes)]],na.rm=TRUE) }
  ...
  list(campaign.profit.curve = campaign.profit.curve,
       ...
       targetsize.maxprofit.trimmed = targetsize.maxprofit.trimmed, # originally called opttargetsize.trimmed
